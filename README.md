...
- **🎲 GM** — build and run campaigns. The **Campaign & Session Manager** is now built: create campaigns, each
  with a **party** of player characters (add characters saved on this device — each shows Soul Level, HP/KP and
  XP progress, and opens straight to its live sheet; **award XP** to one character or the whole party), a **Loot
  menu** (browse/search the full item catalog and drop items straight into a character's inventory — plus custom
  note-only loot for coin and story items), an **Encounter Builder** (assemble fights from the Bestiary and see a
  live difficulty readout — Trivial → Deadly — weighed against your party's Soul Levels), a premise and freeform GM
  notes, a dated **session log** (recaps so you can
  ...
  Encounter-building and a live combat/initiative tracker are next.
- **📖 Codex** — a **searchable reference** for the whole game, **now built**. Browse or search across
  **Kinetics, Techniques, Fusion Kinetics, Otherkin, Backgrounds, Heritages, Skills, Combat Skills, Conditions,
  ...
  its parent Kinetics). The **Bestiary** now holds **50 creatures** from real-world myth & legend, organized as a
  ...
  creatures are treated as **animal species** with a natural-history entry (abundance, social, diet, activity, and
  ...
  **huntable** (🏹 — quarry for meat, hide, or reagents), and any of them drops straight onto a character's **🐾
  ...
  the traditional **Vampire** — with the Lycan and Strigoi appearing here in the flesh *and* as Otherkin. Three of
...
- **Character Creator** — full step-by-step flow:
  ...
  2. **Regional Heritage** — your character's **ancestral lineage** (replaces a race system). A heritage is the
     ...
     *(The roster is being rebuilt toward the final design — a fresh set of peoples, **one at a time**, with
     ...
     Islander**, **The Andes**, **The Amazon**, **The Franks**, **The Norse**, **Rome**, **The Celts**, **The Hellenes**, **Iberia**, **The Slavs**, **The Carpathians**, and **Yamato** (Japan); more are in development, region by region.)* Each grants a
...
- **Live Play Sheet** (▶ Play on a character) — the at-the-table screen, organized into tabs so there's
  ...
  - **♥ Otherkin** — at **Soul Level 15** the Heart chakra opens and your **Soul Creature** awakens. You choose it
    ...
    3rd level. The **♥ Otherkin tab** on the play sheet then shows your unlocked and upcoming powers plus the
    signature's rest-gated uses; the techniques are also playable from Combat. Some signatures are **toggled states** — either a **physical transformation** (a beast-form with stat buffs and natural weapons/armor) or an **invocation** (you gain a creature's abilities without changing shape). *(In the app so far: **Kitsune** — Fox Mischief, **Siren** — Seductive Voice, **Gryphon** — Sovereign Wing, **Lycan** — Lycanthropy, **Troll** — Troll's Fury, **Unicorn** — Mystic Grace, **Sphynx** — Sphynx Riddles, **Wyvern** — Draconic Fire, and **Strigoi** — Blood Rites. All nine are in.)*
  ...
  - **Inventory tab** — carry weight, carried items (equip, actions, config), a custom-item form, and a
    **🔍 Browse Item Catalog** button that opens a dedicated **catalog screen**: search/filter the item
    catalog (weapons + armor, shields, **ammunition**, consumables, tools, gear) and Add any of them (weapons
